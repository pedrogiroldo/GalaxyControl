...
export function PowerOnLidOpenSettings() {
  ...
  const loadCurrentSetting = async () => {
    try {
      ...
      setError("Erro ao carregar configuração de power on lid");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      setError("Erro ao alterar configuração de power on lid");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <SettingsSection
      title="Power on Lid Settings"
      description="Control whether the laptop powers on when the lid is opened"
      icon={Power}
    >
      <div className="space-y-6">
        {error && (
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium">Power on Lid</label>
            ...
            <p className="text-xs text-muted-foreground">
              When enabled, opening the laptop lid will automatically power on
              the device.
            </p>
          </div>
        </div>
